...
while curr_row < num_rows:
    ...
    #4 Imprint Methods
    imprint_method = []
    if row[9].value and row[9].value.strip() not in imprint_method and row[9].value != 'N/A':
        imprint_method.append(row[9].value.strip())
    if row[10].value and row[10].value.strip() not in imprint_method and row[10].value != 'N/A':
        imprint_method.append(row[10].value.strip())
    if row[11].value and row[11].value.strip() not in imprint_method and row[11].value != 'N/A':
        imprint_method.append(row[11].value.strip())
    if row[12].value and row[12].value.strip() not in imprint_method and row[12].value != 'N/A':
        imprint_method.append(row[12].value.strip())

    #4 Imprint Positions
    imprint_position = []
    if row[17].value and row[17].value.strip() not in imprint_position and row[17].value != 'N/A':
        imprint_position.append(row[17].value.strip())
    if row[18].value and row[18].value.strip() not in imprint_position and row[18].value != 'N/A':
        imprint_position.append(row[18].value.strip())
    if row[19].value and row[19].value.strip() not in imprint_position and row[19].value != 'N/A':
        imprint_position.append(row[19].value.strip())
    if row[20].value and row[20].value.strip() not in imprint_position and row[20].value != 'N/A':
        imprint_position.append(row[20].value.strip())
    ...
    if prod_tmpl_name in my_data.keys():
        ...
        variant_list.append({
            ...
            'sku': row[0].value and row[0].value.strip(),
            'color': color,
            'size': size
        })
...
for prod_tmpl_name in my_data.keys():
    ...
    prod_tmpl_id = sock.execute(dbname, uid, pwd, 'product.template', 'search', [('name', '=', prod_tmpl_name), ('sale_ok', '=', True)])
    if prod_tmpl_id:
    ...
    # if prod_tmpl_id and product_color and product_size:
        ...
        if '--imprint_method_position' in args:
            ...
            for imprint_method in tmpl_data.get('imprint_method',[]):
                ...
                if not imprint_id:
                    imprint_id = [sock.execute(dbname, uid, pwd, 'product.variant.dimension.type', 'create', {'name': imprint_method.strip(), 'attribute_field_type': 'none'})]
