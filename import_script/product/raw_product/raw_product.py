...
# sock_common = xmlrpclib.ServerProxy('https://bma8.officebrain.com/xmlrpc/common')
# sock = xmlrpclib.ServerProxy('https://bma8.officebrain.com/xmlrpc/object')
...
while curr_row < num_rows:
    ...
    prod_tmpl_name = row[4].value.strip().title()
    print "\n\ncurrent row is:::",curr_row

    #4 Imprint Methods
    # imprint_method = []
    # if row[6].value and row[6].value not in imprint_method and row[6].value != 'N/A':
    #     imprint_method.append(row[6].value)
    # if row[7].value and row[7].value not in imprint_method and row[7].value != 'N/A':
    #     imprint_method.append(row[7].value)
    # if row[8].value and row[8].value not in imprint_method and row[8].value != 'N/A':
    #     imprint_method.append(row[8].value)
    # if row[9].value and row[9].value not in imprint_method and row[9].value != 'N/A':
    #     imprint_method.append(row[9].value)

    #4 Imprint Positions
    # imprint_position = []
    # if row[14].value and row[14].value not in imprint_position and row[14].value != 'N/A':
    #     imprint_position.append(row[14].value)
    # if row[15].value and row[15].value not in imprint_position and row[15].value != 'N/A':
    #     imprint_position.append(row[15].value)
    # if row[16].value and row[16].value not in imprint_position and row[16].value != 'N/A':
    #     imprint_position.append(row[16].value)
    # if row[17].value and row[17].value not in imprint_position and row[17].value != 'N/A':
    #     imprint_position.append(row[17].value)

    color = []
    size = []
    if row[27].value.strip() and row[27].value.strip() != 'N/A':
        color = sock.execute(dbname, uid, pwd, 'product.attribute.value', 'search', [('name','=',row[27].value.strip().title()), ('attribute_id','=',color_attribute_id)])
        if not color:
            sock.execute(dbname, uid, pwd, 'product.attribute.value', 'create', {'name': row[27].value.strip().title(), 'attribute_id':color_attribute_id})
    if row[25].value.strip() and row[25].value.strip() != 'N/A':
        size = sock.execute(dbname, uid, pwd, 'product.attribute.value', 'search', [('name','=',row[25].value.strip().title()), ('attribute_id','=',size_attribute_id)])
        if not size:
            sock.execute(dbname, uid, pwd, 'product.attribute.value', 'create', {'name': row[25].value.strip().title(), 'attribute_id': size_attribute_id})
    ...
    if prod_tmpl_name in my_data.keys():
        ...
        # if row[10].value:
        #      prod_tmpl_data.update({'setup_charge': row[10].value})
        #Attribute Max Value/Imprint Colors Available
        # if row[13].value:
        #     prod_tmpl_data.update({'attribute_max_value': row[13].value})
        ...
        # if row[19].value:
        #     prod_tmpl_data.update({'run_charge': row[19].value})
        ...
        categ_id = sock.execute(dbname, uid, pwd, 'product.category', 'search', [('name','=',row[6].value.strip().title())])
        if not categ_id:
            categ_id = [sock.execute(dbname, uid, pwd, 'product.category', 'create', {'name': row[6].value.strip().title()})]
        my_data.update({
            prod_tmpl_name:{
                'categ_id': categ_id and categ_id[0],
                # 'imprint_method': imprint_method,
                # 'imprint_position': imprint_position,
                # 'setup_charge': row[10].value,
                # 'run_charge': row[19].value,
                # 'attribute_max_value': row[13].value,
                #'ltm_charge': row[29].value,
                #'min_qty_ltm': row[28].value,
                'variant': [
                    {
                        'desc': row[5].value or False,
                        'sku': row[3].value or False,
                        'color': color,
                        'size': size,
                    }
                ] 
    ...
    # print "mydddataatatata",my_data

for prod_tmpl_name in my_data.keys():
    
    prod_tmpl_id = sock.execute(dbname, uid, pwd, 'product.template', 'search', [('name', '=', prod_tmpl_name),('is_variant','=',False)])
    if prod_tmpl_id:
    ...
    # if prod_tmpl_id and product_color and product_size:
        template_vals = {
            ...
            #'standard_price': float(str(row[2].value)) or False,#Cost Price
            'sale_delay': 1,
            'cost_method': 'real',
            'sale_ok':False,
            'purchase_ok':True,
        ...
        #         if not imprint_id:
        #             imprint_id = [sock.execute(dbname, uid, pwd, 'product.variant.dimension.type', 'create', {'name':imprint_method, 'attribute_field_type': 'none'})]
        ...
        #             product_dim_type_vals.append((0, 0, color_vals))
            ...
            #         imp_pos_id = sock.execute(dbname, uid, pwd, 'product.variant.dimension.option', 'search', [('name', '=', imprint_position), 
            #             ('dimension_id', 'in', imprint_pos_dim_id)])
            #         if not imp_pos_id:
            #             imp_pos_id = [sock.execute(dbname, uid, pwd, 'product.variant.dimension.option', 'create', {'name': imprint_position, 'dimension_id': imprint_pos_dim_id[0]})]
            #         product_dimension_option_id.append(imp_pos_id[0])
                ...
                #         'attribute_field_type': 'multiselection', 'max_free_attributes': 1, 'per_dim_value': True, 'per_qty': True})]})#'product_dimension_type_id':
                    # print "\nRun Charge :D"
    ...
    for variant in my_data[prod_tmpl_name]['variant']:
        ...
        if color_id and size_id:
            product_id = sock.execute(dbname, uid, pwd, 'product.product', 'search', [('name','=',prod_tmpl_name.strip().title()),
                ...
                ('attribute_value_ids', 'in', [color_id[0]]) , ('attribute_value_ids', 'in', [size_id[0]]) ])
        elif color_id and not size_id:
            product_id = sock.execute(dbname, uid, pwd, 'product.product', 'search', [('name','=',prod_tmpl_name.strip().title()),
                ('sale_ok', '=', False),('purchase_ok','=',True), 
                ('attribute_value_ids', 'in', color_id)])
        elif not color_id and size_id:
            product_id = sock.execute(dbname, uid, pwd, 'product.product', 'search', [('name','=',prod_tmpl_name.strip().title()),
                ('sale_ok', '=', False),('purchase_ok','=',True),
                ('attribute_value_ids', 'in', size_id)])
