...
while curr_row < num_rows:
    ...
    if prod_tmpl_name in my_data.keys():
        ...
        if row[11].value:
             prod_tmpl_data.update({'setup_charge': row[11].value})
        #Attribute Max Value/Imprint Colors Available
        if row[14].value:
            prod_tmpl_data.update({'attribute_max_value': row[14].value})
        ...
        if row[20].value:
            prod_tmpl_data.update({'run_charge': row[20].value})
        ...
        my_data.update({
            prod_tmpl_name:{
                ...
                'setup_charge': row[11].value,
                'run_charge': row[20].value,
                'attribute_max_value': row[14].value,
                #'ltm_charge': row[29].value,
                #'min_qty_ltm': row[28].value,
                'variant': [
                    {
                        'desc': row[5].value or False,
                        'sku': row[0].value or False,
                        'color': color,
                        'size': size,
                    }
                ] 
...
for prod_tmpl_name in my_data.keys():
    ...
    prod_tmpl_id = sock.execute(dbname, uid, pwd, 'product.template', 'search', [('name', '=', prod_tmpl_name)])
    if prod_tmpl_id:
