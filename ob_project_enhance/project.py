...
class project_project(models.Model):
    ...
    def set_planning(self, cr, uid, ids, context=None):
        return self.write(cr, uid, ids, {'state': 'planning'}, context=context)
    ...
    state = fields.Selection([('template', 'Template'),
                                   ...
                                   ('pending','Pending'),
                                   ('close','Closed')],
                                  'Status', required=True, copy=False, default='planning')
    stage = fields.Selection([('pre_sales','Presales'),
                                   ('analysis','Analysis'),
                                   ('planning','Planning'),
                                   ...
                                   ('on_halt','On Halt'),
                                   ('completed','Completed')],
                                  'Stage', default='pre_sales')
    quotation = fields.Char('Quotation')
    type_of_payment = fields.Selection([('fix_price', 'Fix Price'),
                                   ('resource_based','Resource Based (Monthly)')],
                                  'Type Of Payment', default='fix_price')
...
class project_task_work(models.Model):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.has_key('hours') and vals['hours'] and task_sequence_id:
            ...
            print "task_sequence_id.task_estimated_id.id===========",task_sequence_id.task_estimated_id.id
...
#        return super(task, self).create(vals)
    ...
    def create(self, vals):
        ...
        d = datetime.strptime(vals['date'], '%Y-%m-%d %H:%M:%S')
        vals['date_filter'] = d.strftime('%Y-%m-%d')
        vals['year'] = d.strftime('%Y')
        if vals['hours'] and vals['task_sequence_id']:
            est_id = self.env['project.task.sequence'].browse(vals['task_sequence_id']).task_estimated_id
            hours = float(est_id.total_time_spent) + float(vals['hours'])
            est_id.write({'total_time_spent':hours})
    ...
    def _onchange_task_sequence_id(self):
        ids = self.task_sequence_id.id
        task_number = self.env['project.task.sequence'].search([('id','=',self.task_sequence_id.id)]).name
        task_estimated_id = self.env['task.estimated'].search([('task_number','=',task_number)])
        estimated_task_type_id = task_estimated_id.estimated_task_type_id
        self.estimated_task_type_id = estimated_task_type_id
...
class task_estimated(models.Model):
    ...
    def _get_hours(self):
        if self.id:
            project_task_sequence_id = self.env['project.task.sequence'].search([('task_estimated_id', '=', self.id)]).id
            res = self.env['project.task.work'].search([('task_sequence_id', '=', project_task_sequence_id)])
            hours = 0
            for ids in res:
                hours = hours + ids.hours
            self.total_time_spent=hours    
        return True
    ...
    total_time_spent =  fields.Float('Total Time Spent', compute='_get_hours')
    ...
    work_ids = fields.Many2one('project.task.work', 'Project Task Work')

class project_sprint(models.Model):
    _name = "project.sprint"

    name = fields.Text('Description')
    sprint_start_date= fields.Date('Phase Start Date')
    sprint_end_date= fields.Date('Phase End Date')
...
#            self.project_id.write({'time_spent':hours})
    ...
    @api.multi
    @api.depends('estimated_id')
    def _estimated_total_hours(self):
        task_id = self.id
        if task_id:
            self._cr.execute("SELECT SUM(hours) FROM task_estimated WHERE estimated_task_id=%s", [task_id])
            res = self._cr.dictfetchone()
            hours = 0
            if res.get('sum'):
                hours = float(res['sum'])
    ...
    sprint_started = fields.Many2one('project.sprint', string='Phase',track_visibility='onchange')
    ...
    planned_hours = fields.Float(string='Actual Planned Hour(s)', compute='_estimated_total_hours')
