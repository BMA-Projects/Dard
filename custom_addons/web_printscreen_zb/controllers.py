...
class ZbExcelExport(ExcelExport):
    ...
    def from_data_custom(self, fields, rows):
    ...
    def index_custom(self, req, data, token):
        data = json.loads(data)
        return req.make_response(
            self.from_data_custom(data.get('headers', []), data.get('rows', [])),
                           headers=[
...
class ExportPdf(Export):
    ...
    def from_data(self, uid, fields, rows, company_name, company_logo, tagline, username, report_lable):
        ...
        _append_node('image', company_logo)
        _append_node('tagline',tagline)
        _append_node('user',username)
#         _append_node('header-date',company_image)
        _append_node('company', company_name)
        _append_node('header-date', time.strftime(str(locale.nl_langinfo(locale.D_FMT).replace('%y', '%Y'))))
        _append_node('report_lable',report_lable)
...
#         image = etree.SubElement(image,'image'/)
        i = 0
        for f in fields:
        ...
        transform = etree.XSLT(
            etree.parse(os.path.join(tools.config['root_path'],
                                   'addons/web_printscreen_zb/report/custom_new.xsl')))
...
class ZbPdfExport(ExportPdf):
    ...
    def index(self, req, data, token):
        ...
        return req.make_response(self.from_data(uid, data.get('headers', []), data.get('rows', []),
                                                data.get('company_name',''), data.get('company_logo'),
                                                data.get('tagline',''), data.get('username',''),
                                                data.get('rpt_label','')),
