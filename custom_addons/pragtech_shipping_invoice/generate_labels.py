...
#         self.pool.get('generate.labels_details').create(cr, uid, item, context=None)
    ...
    def generate_shipping_report(self,cr, uid, ids, context=None):
        ...
        vals['shipping_carrier'] = sp_browse_obj.carrier_id.id
        vals['delivery_date'] = sp_browse_obj.min_date
        vals['delivery_order'] = sp_browse_obj.name
        vals['sale_order'] = sp_browse_obj.sale_id.name
        vals['state'] = sp_browse_obj.state
        so_browse_obj = self.pool.get('sale.order').browse(cr,uid,sp_browse_obj.sale_id.id)
        vals['customer'] = so_browse_obj.partner_id.id
        vals['sale_order_date'] = so_browse_obj.date_order
        rp_browse_obj = self.pool.get('res.partner').browse(cr,uid,so_browse_obj.partner_id.id)
        ...
        if self.pool.get('shipping.report').create(cr, uid, vals):
            print "new record created in shipping.report table"
            return True
...
#                     url = url = "https://stg-secure.shippingapis.com/ShippingApi.dll?API=DelivConfirmCertifyV4&" if shippingusps_ptr.test else "http://stg-production.shippingapis.com/ShippingApi.dll?API=DelivConfirmCertifyV4&"
