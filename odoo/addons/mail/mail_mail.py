...
class mail_mail(osv.Model):
    ...
    def create(self, cr, uid, values, context=None):
        '''
        Modifield the method to send message directly it will shoot mail directly: E_ID : 4616        
        '''
        if 'notification' not in values and values.get('mail_message_id'):
            values['notification'] = True
        vals = super(mail_mail, self).create(cr, uid, values, context=context)
        return vals
    ...
    def process_email_queue(self, cr, uid, ids=None, context=None):
        ...
        if not ids:
            ...
            ids = self.search(cr, uid, filters, context=context, limit=250)
        res = None
        try:
