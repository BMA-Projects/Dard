...
class ir_ui_menu(osv.osv):
    ...
    def _filter_visible_menus(self):
        ...
        with self._menu_cache_lock:
            ...
            module_install = self.env['ir.module.module'].sudo().search([('name','=','ob_menu_level_access_rights'),('state','=','installed')])
            ...
            if key in self._menu_cache:
                ...
                for menu in action_menus:
                    ...
                    if not fname or not menu.action[fname] or \
                        ...
                        if not module_install:
                            ...
                            if not set(menu.invisible_groups.ids) & set(groups.ids):
                                visible += menu
                ...
                if module_install:
                    for menu in menus:
                        if set(menu.invisible_groups.ids) & set(groups.ids):
                            invisible_menu.append(menu.id)
                    visible_ids_list = list(visible._ids)
                    for menu in invisible_menu:
                        if menu in visible_ids_list:
                            visible_ids_list.remove(menu)
                if module_install:
                    self._menu_cache[key] = tuple(visible_ids_list)
                    visible = self.browse(visible_ids_list)
                else:
    ...
    def write(self, cr, uid, ids, values, context=None):
        # Method changed, Now update the module then not add groups which is deleted from frontend
        if values.get('groups_id', False):
            menu_rec = self.browse(cr, uid, ids, context=context)
            frontend_groups = menu_rec.groups_id and menu_rec.groups_id.ids or []
            full_list = frontend_groups if context.get('install_mode', False) else values.get('groups_id')
            values.update({'groups_id': full_list})
        self.clear_cache()
