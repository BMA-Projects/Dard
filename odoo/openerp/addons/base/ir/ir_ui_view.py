...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if context and not context.get('install_mode'):
            model_obj = self.pool.get('ir.model.data')
            model_ids = model_obj.search(cr, uid, [('res_id', 'in', ids), ('model','=', self._name)])
            model_obj.write(cr, uid, model_ids, {'noupdate': True}, context=context)
        ...
        if vals.get('groups_id', False):
            ...
            full_list = frontend_groups if context.get('install_mode', False) else vals.get('groups_id')
            vals.update({'groups_id': full_list})
