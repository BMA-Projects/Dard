...
class ir_model_access(osv.osv):
    ...
    def check(self, cr, uid, model, mode='read', raise_exception=True, context=None):
        ...
        # FIRST CHECK RULEWISE
        cr.execute('SELECT a.group_id '
                   '  FROM ir_model_access a '
                   '  JOIN ir_model m ON (m.id = a.model_id) '
                   ...
                   '    AND a.group_id in (select id from res_groups where x_is_custom_group is True and id=a.group_id) '
                   '  ORDER BY a.group_id DESC '
                   , (model_name, uid,)
                   )
        fetched_group = cr.fetchone()
        if fetched_group:
            cr.execute('SELECT MAX(CASE WHEN perm_' + mode + ' THEN 1 ELSE 0 END) '
                       ...
                       '   AND a.active IS True '
                       '   AND a.group_id = %s '
                       , (model_name, uid, fetched_group[0])
                       )
            t = cr.fetchone()
            r = t and t[0] or None
        else:
