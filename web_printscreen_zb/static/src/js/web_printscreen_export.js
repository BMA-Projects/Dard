        ...
        export_to_excel: function(export_type) {
            ...
            $.each(header_eles,function(){
                ...
                $.each(header_td_elements,function(){
                    ...
                    if (!$header_td.hasClass('oe_button_expand')){
                ...
                if ($data_ele.text().trim()){
	                ...
	                $.each(data_td_eles,function(){
	                    ...
	                    text = '';
	                    	...
	                    	if($data_td_ele && !$data_td_ele[0].classList.contains('oe-button') && !$data_td_ele[0].classList.contains('oe_row_add')){
	                        	text = $data_td_ele.text().trim() || "N/A";
	                    	}
	                ...
	                if (!$data_ele.hasClass( "oe_group_header" ) && $data_ele.find('.oe_list_record_selector > input') && $data_ele.find('.oe_list_record_selector > input')[0].checked){
	                    export_data.push(data);
    	                is_filled = true;
	                }
                    /*if ($data_ele[0].children[1].children[0]){
    	                if ($data_ele[0].children[1].children[0].checked){
    	                	export_data.push(data)
    	                	is_filled = true;
    	                }
                    }
                    if ($data_ele[0].children[0].children[0]) {
                         if ($data_ele[0].children[0].children[0].checked){
                            export_data.push(data)
                            is_filled = true;
                        }
                    }*/
            ...
            $.each(footer_eles,function(){
                ...
                $.each(footer_td_eles,function(){
                    $footer_td_ele = $(this)
                    if(!($footer_td_ele && $footer_td_ele[0].classList.length>0 && $footer_td_ele[0].classList.contains('oe_expand_footer'))){
                        text = $footer_td_ele.text().trim() || ""
                    }
                    if ($footer_td_ele && $footer_td_ele[0].classList.contains('oe_number')){
                    	text_type = typeof text;
                    	if (text_type = 'string')
                    		text_type = "float_time";
                        text = instance.web.parse_value(text, {type : text_type})
                        data.push({'data': text || "", 'bold': true, 'number': true})
                    }
                    else{
                        if($footer_td_ele && $footer_td_ele[0].classList.length ==0 || $footer_td_ele && $footer_td_ele[0].classList.contains('oe_list_footer')){
                            data.push({'data': text, 'bold': true})
                        }
                    }
                });
            ...
            if (export_type === 'excel'){
            	if(is_filled){
                    ...
                    }, $('<div>').html('You must choose at least one record.')).open();
                }
             }
             else{
                new instance.web.Model("res.users").get_func("read")(this.session.uid, ["company_id", "name"]).then(function(res) {
                    new instance.web.Model("res.company").get_func("read")([res['company_id'][0]]).then(function(result) {
                        view.session.get_file({
