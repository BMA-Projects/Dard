		...
		*/
		function showLightbox(imageObject) {
			...
			**/
			...
			imageNum = 0;
			if (opts.activeImage!=null){
				imageNum = opts.activeImage; 
			}
			...
			if (opts.imageArray.length > 1) {
				...
				// console.log("imageObject",imageObject,opts.imageArray[imageNum]);
				// while (opts.imageArray[imageNum][0] != imageObject.href) {
					// console.log("imageNumBefore.....",imageNum);
					// imageNum++;
					// console.log("imageNumAfter.....",imageNum);
		...
		function doChangeImage() {
			...
			imgPreloader.onload = function() {
				...
				$("#imageContainer").attr("style", "line-height:" + "30");
				$("#lightboxImage").attr("style", "line-height:" + "30");
				$('#lightboxImage').
			...
			function preloadNeighborImages() {
				if (opts.loopImages && opts.imageArray.length > 1) {
					...
					preloadNextImage.src = opts.imageArray[(opts.activeImage == (opts.imageArray.length - 1)) ? 0 : (parseInt(opts.activeImage) + 1)][0];
					preloadPrevImage = new Image();
					preloadPrevImage.src = opts.imageArray[(opts.activeImage == 0) ? (opts.imageArray.length - 1) : (parseInt(opts.activeImage) - 1)][0];
				} else {
				...
				if (opts.imageArray.length > 1) {
					...
					if(opts.loopImages) {
						$('#prevLink,#prevLinkText').show().click(function() {
							changeImage((opts.activeImage == 0) ? (opts.imageArray.length - 1) : (parseInt(opts.activeImage) - 1)); 
							return false;
						});

						$('#nextLink,#nextLinkText').show().click(function() {
							changeImage((opts.activeImage == (opts.imageArray.length - 1)) ? 0 : (parseInt(opts.activeImage) + 1)); 
							return false;
						});
					} else {
		...
		$.fn.lightbox.defaults = {
		    triggerEvent: "click",
			allSet: false,
			fileLoadingImage: '/ob_product_multi_images/static/lib/lightbox/images/loading.gif',
			fileBottomNavCloseImage: '/ob_product_multi_images/static/lib/lightbox/images/closelabel.gif',
			overlayOpacity: 0.6,
			borderSize: 10,
